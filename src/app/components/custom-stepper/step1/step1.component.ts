...
export class Step1Component implements OnInit, OnChanges {
  ...
  @Input() showEmailPasswordInputs: boolean = true;
  ...
  ngOnInit() {
    ...
    if (step1Data && Object.keys(step1Data).length > 0) {
      ...
      if (step1Data.fullName !== undefined)
        formData['fullName'] = step1Data.fullName;
      if (step1Data.birthdateDay !== undefined)
        formData['birthdateDay'] = step1Data.birthdateDay;
      if (step1Data.birthdateMonth !== undefined)
        formData['birthdateMonth'] = step1Data.birthdateMonth;
      if (step1Data.birthdateYear !== undefined)
        formData['birthdateYear'] = step1Data.birthdateYear;
      if (step1Data.parentRole !== undefined)
        formData['parentRole'] = step1Data.parentRole;
      if (step1Data.email !== undefined) formData['email'] = step1Data.email;
      if (step1Data.password !== undefined)
        formData['password'] = step1Data.password;
      if (step1Data.profilePicture !== undefined)
        formData['profilePicture'] = step1Data.profilePicture;
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['showEmailPasswordInputs']) {
      this.setEmailPasswordValidators();
    }
  }

  setEmailPasswordValidators() {
    const emailControl = this.form.get('email');
    const passwordControl = this.form.get('password');

    if (this.showEmailPasswordInputs) {
